...
import os, re, math, sys, subprocess, argparse
...
def make_coloring(out, cats, title):
    ...
    T(c, "给图标涂色，下面写一写单词", A4W-24, A4H-30, 12, (255,255,255), align="r", bold=False)
    y=A4H-80
    panel_h=118
    for ci,(cat,items) in enumerate(cats):
        col=colors.HexColor(PALETTE[ci%len(PALETTE)])
        ...
        for (word,cp,zh) in items[:n]:
            im=emoji_img(cp); d=54
            c.drawImage(ImageReader(im), bx, y-panel_h+16, d, d, mask="auto")
            T(c, word, bx+d/2, y-panel_h+4, 9, (0,0,0), align="c")
            bx += d+36
        y-=panel_h+18
        if y<panel_h+60:
